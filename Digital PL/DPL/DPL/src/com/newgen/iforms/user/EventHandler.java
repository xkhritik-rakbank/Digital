...
public class EventHandler extends Digital_PL_Common  implements IFormServerEventHandler
{	
	...
	{	
		Digital_PL.mLogger.info("Inside executeServerEvent");
		Digital_PL.mLogger.info("Inside executeServerEvent() ak 101 ---control: " + control + "\nevent: " + event + "\nStringData: "
				+ Stringdata);
		...
		Digital_PL.mLogger.debug("Event = "+event);
		if("click".equalsIgnoreCase(event))
		{
			try {
				return new Digital_PL_Click().clickEvent(iformObj,control,event,Stringdata);
		...
		{
			return new Digital_PL_FormLoad().formLoadEvent(iformObj,control,event,Stringdata);
		}
		else if("introducedone".equalsIgnoreCase(event))
		{
			return new Digital_PL_IntroDone().onIntroduceDone(iformObj,control,event,Stringdata);
		}
		else if("Change".equalsIgnoreCase(event))
		{
			Digital_PL.mLogger.debug("Eventhandler . Change event");
			return new Digital_PL_Change().changeEvent(iformObj,control,event,Stringdata);
		}
		else if("ReadOnly".equalsIgnoreCase(event))
		{
			return new Digital_PL_ReadOnly().onevent(iformObj,control,event,Stringdata);
		}
