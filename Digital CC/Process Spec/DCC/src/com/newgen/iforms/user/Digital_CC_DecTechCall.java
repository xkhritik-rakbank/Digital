...
public class Digital_CC_DecTechCall extends Digital_CC_Common {
	...
	public String onevent(IFormReference iformObj, String control, String StringData) throws IOException {
		...
		Digital_CC.mLogger.debug("Output_Eligible_Card: "+Output_Eligible_Card+ "WI: "+wiName);
		
		String[] Output_Eligible_Cards_Arr=Output_Eligible_Card.split("\\},\\{");
		String card_Product = "";
		
		Digital_CC.mLogger.debug("value of Output_Eligible_Cards_Arr : "+Output_Eligible_Cards_Arr);
		Digital_CC.mLogger.debug("value of Output_Eligible_Cards_Arr length : "+Output_Eligible_Cards_Arr.length);
		
		try{
			if(Output_Eligible_Cards_Arr.length>0){
				
				for (int i=0; i<Output_Eligible_Cards_Arr.length;i++){
					Digital_CC.mLogger.debug("value of i: "+i);
					...
					if(Output_Eligible_Cards_Array.length==3){
						...
						Digital_CC.mLogger.debug("Output_Eligible_Cards_Product:"+CArdProducyList[0]+" "+CArdProducyList[1]);
						
						if(!card_Product.isEmpty()){
							Digital_CC.mLogger.debug("Output_Eligible_Cards if block:"+card_Product);
							card_Product = card_Product+","+CArdProducyList[1];
						}else{
							Digital_CC.mLogger.debug("Output_Eligible_Cards else block:"+card_Product);
			...
			Digital_CC.mLogger.debug("error msg "+e.getMessage());
		}
		Digital_CC.mLogger.debug("card_Product: "+card_Product+ "WI: "+wiName);
		card_Product = card_Product.replaceAll("\"", "");
		card_Product = card_Product.replaceAll(",", "~");
		String OutputAlternateCard = xmlParserSocketDetails.getValueOf("Output_Alternate_Card"); 
		Digital_CC.mLogger.debug("OutputAlternateCard: "+OutputAlternateCard+ "WI: "+wiName);
		
		// Added Net_Salary1,Net_Salary2,Net_Salary3 - Added on 28.12.23 PDSC-1303
		// AECB_Sal_Month_1	AECB_Sal_Month_2	AECB_Sal_Month_3   - PDSC-1376
		...
		try{
			...
			Digital_CC.mLogger.debug("Output_AECB_Salary_1: "+Output_AECB_Salary_1+ "WI: "+wiName);
			Digital_CC.mLogger.debug("Output_AECB_Salary_2: "+Output_AECB_Salary_2+ "WI: "+wiName);
			Digital_CC.mLogger.debug("Output_AECB_Salary_3: "+Output_AECB_Salary_3+ "WI: "+wiName);
			...
			Digital_CC.mLogger.debug("columnValues: "+columnNames+ " WI: "+wiName);
			Digital_CC.mLogger.debug("columnValues: "+columnValues+ " WI: "+wiName);
			
		}catch(Exception e){
			Digital_CC.mLogger.debug("Exception Output AECB Salary : " + e.getMessage());
		}
		
		String sWhereClause = "WI_NAME='" + wiName + "'";
    	String tableName = "NG_DCC_EXTTABLE";
    	columnNames = columnNames + "Output_Eligible_Card,OutputAlternateCard";
    	columnValues = columnValues+ "'"+card_Product+"','"+OutputAlternateCard+"'";
    	
    	Digital_CC.mLogger.debug("columnValues: "+columnNames+ " WI: "+wiName);
    	Digital_CC.mLogger.debug("columnValues: "+columnValues+ " WI: "+wiName);
    	
        String inputXML = apUpdateInput(getCabinetName(iformObj), getSessionId(iformObj),tableName, columnNames, columnValues, sWhereClause);
        Digital_CC.mLogger.debug("Input XML for apUpdateInput for " + tableName + " Table : " + inputXML);
        String outputXml = WFNGExecute(inputXML, iformObj.getServerIp(), iformObj.getServerPort(), 1);
        ...
        if (!StrMainCode.equals("0")){
        	returnValue="Failed";
        	Digital_CC.mLogger.debug("Error in Executing apUpdateInput sOutputXML for dectech field update: " + outputXml);
        }
	...
	public String DecTechInputBodyXml(IFormReference iformObj, String control, String Data) {
		...
		String DBQuery = "SELECT Wi_Name, Application_Type, uw_income, CIF, isnull(Prefer_product,Product) as 'Product', Product_Desc, Sub_Product, Card_Product, CUSTOMERNAME, PassportNo, EmirateID, MobileNo, employercode, "
				...
				+ "Pensioner,Name_match,FCU_indicator,UW_reqd, requested_limit, Industry, Sub_Industry, FTS_Ack_flg, EFR_NSTP,EStatementFlag,LOB,FTS_FLAG,NTB"
		...
		String[] columns = { "Wi_Name", "Application_Type", "uw_income", "CIF", "Product", "Product_Desc", "Sub_Product",
				...
				"requested_limit","Industry","Sub_Industry", "FTS_Ack_flg", "EFR_NSTP","EStatementFlag","LOB","FTS_FLAG","NTB" };

		Map<String, String> ApplicantDetails_Map = getDataFromDB(DBQuery, cabinetName, sessionId, jtsIp, jtsPort,columns);
		Digital_CC.mLogger.debug("DCC ApplicantDetails_Map : " + ApplicantDetails_Map);
		...
		Map<String, String> ApplicationDetailsOldSalary_Map = getDataFromDB(DBQuery_OldSalary, cabinetName, sessionId, jtsIp, jtsPort, columns_OldSalary);
		//End
		...
		Digital_CC.mLogger.debug("DCC Product : " + validateValue(ApplicantDetails_Map.get("Product")));
		...
		String internal_Bureau = sInputXmlInternalBureau(ApplicantDetails_Map, cabinetName, sessionId, jtsIp, jtsPort);
		...
		/** External Bureau Pipeline Products sub-tag **/
		// Hritik - 09.01.24 -  PDSC-1303 PDSC-1436
		Digital_CC.mLogger.debug("DCC FTS_FLAG : "+ validateValue(ApplicantDetails_Map.get("FTS_FLAG")));
	...
	private static String sInputXmlApplicantDetails(Map<String, String> applicantDetails_Map, String cabinetName,
		...
		String lob=validateValue(applicantDetails_Map.get("LOB"));
		String employercode = validateValue(applicantDetails_Map.get("employercode"));
		Digital_CC.mLogger.debug("lob in sInputXmlApplicantDetails " +lob);
		...
		String ApplicantDetails_Str ="";		
		ApplicantDetails_Str =  "<ApplicantDetails>" + "" + "<applicant_id>" + applicantDetails_Map.get("Wi_Name") + "</applicant_id>"
				...
				if(!"".equals(lob)){
					ApplicantDetails_Str = ApplicantDetails_Str	+ "<lob>" + lob + "</lob>";// Hritik - 06.09.23	
				}
				ApplicantDetails_Str = ApplicantDetails_Str + "<los>" + CalculatLOS(applicantDetails_Map.get("Date_Of_Joining")) + "</los>" + ""
				...
				Digital_CC.mLogger.debug("ApplicantDetails_Str "+ ApplicantDetails_Str);
		return ApplicantDetails_Str;
	...
	public static String sInputXmlInternalBureau(Map<String, String> applicantDetails_Map , String cabinetName, String sessionID,
			String jtsIP, String jtsPort) {
		...
		Digital_CC.mLogger.debug("InputXmlInternalBureau 1  + "+internal_bureau);
		
		if("false".equals(applicantDetails_Map.get("NTB"))) {
			
			String sQuery = "select Wi_Name,Ext_card_prd_code,Ext_card_prd_desc,Ext_card_prd_type,Ext_card_prd_status,Ext_card_prd_CRN from ng_dcc_gr_active_cards Where Wi_Name='"+Wi_Name+"'";
			Digital_CC.mLogger.debug("sInputXmlInternalBureau sQuery" + sQuery+ "");
			try {
				List<Map<String,String>> OutputXML = getDataFromDBMap(sQuery,cabinetName,sessionID, jtsIP, jtsPort);
				if (OutputXML.size() == 0) {
					Digital_CC.mLogger.debug("OutputXML size sQuery" + OutputXML.size()+ "");	
					...
					for (Map<String,String> map : OutputXML) {
						...
						String Ext_card_prd_CRN="",Ext_card_prd_code="",Ext_card_prd_status="";
						...
						Digital_CC.mLogger.debug("Ext_card_prd_CRN " + Ext_card_prd_CRN+ "");
						Digital_CC.mLogger.debug("Ext_card_prd_code " + Ext_card_prd_code+ "");
						Digital_CC.mLogger.debug(" Ext_card_prd_status " + Ext_card_prd_status+ "");
						...
						Digital_CC.mLogger.debug(" InternalBureauIndividualProducts add_xml_str " + add_xml_str+ "");
					}
				}
			}
			catch (Exception e) {
				Digital_CC.mLogger.debug("DECTECH Exception occurred in sInputXmlInternalBureau()"+ e.getMessage() + " Error: "+ e.getMessage());
				return null;
			}
		}
		else {
		...
		Digital_CC.mLogger.debug("InputXmlInternalBureau 2 + "+internal_bureau);
