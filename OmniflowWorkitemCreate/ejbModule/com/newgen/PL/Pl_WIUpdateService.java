...
public class Pl_WIUpdateService extends CreateWorkitem{
	...
	public WICreateResponse wiUpdate(WICreateRequest request,String attributeList[]) throws WICreateException{
		...
		try
		{
			...
			// Load ResourceBundle
			this.pCodes=commonMethod.loadResourceBundle();
			...
			if ("STMT_ANLYS_PL".equalsIgnoreCase(sSubProcess))
    		{
    			...
    			//remove duplicate entry in IncomeExpense table
    			String incomeExpenseQuert = "select Bills,Lifestyle_Expenses,Other_Commitments from NG_DPL_IncomeExpense with(NOLOCK) where WI_Name = '"+WI+"'";
    			
    			sInputXML=commonMethod.getAPSelectWithColumnNamesXML(incomeExpenseQuert);
    			...
    			checkCallsMainCode(xmlParserData);
    			
    			int iTotalrec=Integer.parseInt(xmlParserData.getValueOf("TotalRetrieved"));
    			WriteLog("External table xmlParserData: "+xmlParserData);
    			
    			String MainCode=commonMethod.getTagValues(sOutputXML, "MainCode");
    			WriteLog("Transaction table MainCode: "+MainCode);
    			
    			if(MainCode.equalsIgnoreCase("0") && iTotalrec>0){
    				String where = "WI_Name='"+hm.get("WINUMBER")+"'";
        			
        			insertIntoExtTable("NG_DPL_IncomeExpense",NetSal_Col,NetSal_val,sSessionID,where);
    			...
    			trTableValue=extColValues+"'Success'";
			...
			else if("FIRCO_ADDN_DATA_PL".equalsIgnoreCase(sSubProcess)){
			...
			else if("STL_ADDN_DOC_PL".equalsIgnoreCase(sSubProcess)){
				trTableColumn=extColNames+"STLDocProvided,Decision";
    			trTableValue=extColValues+"'N','Success'";
    			decisionInHistory = "STL Doc Received";
    			remark=sRemarksAddInfo;
    			String whereClause = "winame='"+hm.get("WINUMBER")+"'";
			...
			else if("PL_Account_update".equalsIgnoreCase(sSubProcess)){
				
				String SolID =  hm.get("Sol_Id"); 
				String Event =  hm.get("Event");
				String CIF =  hm.get("CIF");
				String SalaryIBAN =  hm.get("IBAN");
				String AccountCreationDate =  hm.get("Account_creation_date");
				String SchemeCode =  hm.get("Scheme_Code");
				String CustomerSubSegment =  hm.get("customer_subsegment");
				
				trTableColumn=extColNames+"Decision,SolID,Event,CIF,SalaryIBAN,AccountCreationDate,"
						+ "SchemeCode,CustomerSubSegment";
    			trTableValue=extColValues+"'Success','"+SolID+"','"+Event+"','"+CIF+"','"+SalaryIBAN+"','"+
    					AccountCreationDate+"','"+SchemeCode+"','"+CustomerSubSegment+"'";
    			
    			decisionInHistory = "Account Updated";
    			remark=sRemarksAddInfo;
    			String whereClause = "winame='"+hm.get("WINUMBER")+"'";
			...
			else if("Offer_Letter_Decision".equalsIgnoreCase(sSubProcess)){
				
				String Offer_letter_Cust_Decision =  hm.get("Offer_letter_Cust_Decision"); 
				String Event =  hm.get("Event");
				
				trTableColumn=extColNames+"Decision,Event,OfferLetter_Dec";
    			trTableValue=extColValues+"'"+Offer_letter_Cust_Decision+"','"+Event+"','"+Offer_letter_Cust_Decision+"'";
    			decisionInHistory = "Account Updated";
    			remark=sRemarksAddInfo;
    			String whereClause = "winame='"+hm.get("WINUMBER")+"'";
			...
			else if("NO_BANK_STMT_ANLYS_PL".equalsIgnoreCase(sSubProcess)){
				
				String FTS_error_reason =  hm.get("FTS_error_reason");
				String UW_reqd =  hm.get("UW_reqd");
				String WI =  hm.get("WINUMBER");
				String Prospect_id =  hm.get("Prospect_id");
				
				String NetSal_Col="FTS_error_reason,UW_reqd,Wi_Name";
    			String NetSal_val = "'"+FTS_error_reason+"','"+UW_reqd+"','"+WI+"'";
    			...
    			if(MainCode.equalsIgnoreCase("0") && iTotalrec>0){
    				...
    				//Update data in IncomeExpense table
        			insertIntoExtTable("NG_DPL_IncomeExpense",NetSal_Col,NetSal_val,sSessionID,where);
	...
	private void validateRequestParameters(String attributeList[]) throws WICreateException,Exception
	{
		...
		}else if (PossibleQueues.contains(ActivityName))
		{
			WriteLog("Update request is possible at this queue: "+ActivityName);
		}
		else 
	...
	private void getWorkitemStage(String attributeList[]) throws WICreateException, Exception
	{
	    ...
	    String getTableNameQry = "";
	    
	    if("NO_BANK_STMT_ANLYS_PL".equalsIgnoreCase(sSubProcess)){
	    	processID = "FTS_Hold";
	    }
	    if("FTS_Hold".equalsIgnoreCase(processID)|| "Digital_PL_STL_Hold".equalsIgnoreCase(processID) || "Digital_PL_Acc_Update_Hold".equalsIgnoreCase(processID)|| "Digital_PL_WI_Update_Hold".equalsIgnoreCase(processID)){
	    	getTableNameQry="select ACTIVITYNAME  from QUEUEVIEW with (nolock) where PROCESSINSTANCEID='"+WINumber+"' and var_str18 ='"+processID+"'";
	    }else{
	    	getTableNameQry="select ACTIVITYNAME  from QUEUEVIEW with (nolock) where PROCESSINSTANCEID='"+WINumber+"'";
	    }
	...
	private void getPossibleUpdateQueues() throws WICreateException, Exception
	{
		...
		sInputXML=commonMethod.getAPSelectWithColumnNamesXML(getTableNameQry);
		System.out.println("getTableNameQry"+getTableNameQry);
					...
					|| ((isMandatory.equalsIgnoreCase("N") || isMandatory.equalsIgnoreCase("C")) && attributeValue.trim().equalsIgnoreCase("")))
			{
				if (!extFlag.equalsIgnoreCase("") && transFlag.equalsIgnoreCase(""))
				{
					...
					if(IsIncludedInUpdate.equalsIgnoreCase("Y"))
					{
						...
						if(processID.equalsIgnoreCase("DPL_UPDATE_CIF_CARD_DETAILS") && attributeName.equalsIgnoreCase("Updated_CardType_Desc")){ // Hritik 09.10.23
	...
	{
		this.headerObjResponse = commonMethod.setSuccessParamResponse(sExtra1,sExtra2,sMessageId,sMsgFormat,sMsgVersion,sRequestorChannelId,sRequestorLanguage,sRequestorSecurityInfo,sRequestorUserId);
		response.setEE_EAI_HEADER(headerObjResponse);
		response.setWorkitemNumber(WINumber);
