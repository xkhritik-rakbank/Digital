...
package com.newgen.DigitalAO.ReadCourierFile;
...
public class DAOCourierFile implements Runnable{	
	...
	public void run()
	{
		...
		try
		{
			DAOCourierFileLog.setLogger();
			ngEjbClientFalconDocument = NGEjbClient.getSharedInstance();

			DAOCourierFileLog.mLogger.debug("Connecting to Cabinet.");

			int configReadStatus = readConfig();

			DAOCourierFileLog.mLogger.debug("configReadStatus "+configReadStatus);
			if(configReadStatus !=0)
			{
				DAOCourierFileLog.mLogger.error("Could not Read Config Properties [CMPDocument]");
				return;
			}

			cabinetName = CommonConnection.getCabinetName();
			DAOCourierFileLog.mLogger.debug("Cabinet Name: " + cabinetName);

			jtsIP = CommonConnection.getJTSIP();
			DAOCourierFileLog.mLogger.debug("JTSIP: " + jtsIP);

			jtsPort = CommonConnection.getJTSPort();
			DAOCourierFileLog.mLogger.debug("JTSPORT: " + jtsPort);

			smsPort = CommonConnection.getsSMSPort();
			DAOCourierFileLog.mLogger.debug("SMSPort: " + smsPort);			

			/*sleepIntervalInMin=Integer.parseInt(falconDocumentCofigParamMap.get("SleepIntervalInMin"));
			DAOCourierFileLog.mLogger.debug("SleepIntervalInMin: "+sleepIntervalInMin);

			attributeNames=falconDocumentCofigParamMap.get("AttributeNames").split(",");
			DAOCourierFileLog.mLogger.debug("AttributeNames: " + attributeNames);

			ExternalTable=falconDocumentCofigParamMap.get("ExtTableName");
			DAOCourierFileLog.mLogger.debug("ExternalTable: " + ExternalTable);

			destFilePath=falconDocumentCofigParamMap.get("destFilePath");
			DAOCourierFileLog.mLogger.debug("destFilePath: " + destFilePath);

			ErrorFolder=falconDocumentCofigParamMap.get("failDestFilePath");
			DAOCourierFileLog.mLogger.debug("ErrorFolder: " + ErrorFolder);

			volumeID=falconDocumentCofigParamMap.get("VolumeID");
			DAOCourierFileLog.mLogger.debug("VolumeID: " + volumeID);

			MaxNoOfTries=falconDocumentCofigParamMap.get("MaxNoOfTries");   //Not getting used anywhere Sajan
			DAOCourierFileLog.mLogger.debug("MaxNoOfTries: " + MaxNoOfTries);

			TimeIntervalBetweenTrialsInMin=Integer.parseInt(falconDocumentCofigParamMap.get("TimeIntervalBetweenTrialsInMin"));
			DAOCourierFileLog.mLogger.debug("TimeIntervalBetweenTrialsInMin: " + TimeIntervalBetweenTrialsInMin);*/
			
			sessionId = CommonConnection.getSessionID(DAOCourierFileLog.mLogger, false);
			if(sessionId.trim().equalsIgnoreCase(""))
			{
				DAOCourierFileLog.mLogger.debug("Could Not Connect to Server!");
			}
			else
			{
				DAOCourierFileLog.mLogger.debug("Session ID found: " + sessionId);
				while(true)
				{
		...
		catch (Exception e)
		{
			//e.printStackTrace();
			try{
				DAOCourierFileLog.mLogger.error("Exception Occurred in FALCON Document Thread: "+e+" Now runnning the utility again");
				Thread.sleep(1000);
				startFalconCourierUtility();
			}
			catch(Exception ex){
	...
	private int readConfig()
	{
		...
		catch (Exception e)
		{
			...
			DAOCourierFileLog.mLogger.error("Exception has occured while loading properties file "+e.getMessage());
			return -1 ;			
		}
		return 0;
	}
	...
	{
		DAOCourierFileLog.mLogger.info("ProcessWI function for Falcon Courier Utility started");
		...
		{
			DAOCourierFileLog.mLogger.error("Could Not Get Session ID "+sessionId);
			return;
		}
		...
		for(File file:listFiles){
			...
			for(int k=1;k<100;k++)
			{
				try
				{
					...
					DAOCourierFileLog.mLogger.info("Exception in reading xls "+e.getMessage());
					break;
				}
			}
			fis.close();
		...
		DAOCourierFileLog.mLogger.info("exiting ProcessWI function FALCON Document Utility");
	}
	...
	private String getFinalLimit(String workItemNumber) throws IOException,Exception{
		...
		DAOCourierFileLog.mLogger.info("Input XML to get Final limit is "+sInput);
		
		String strOutput=WFNGExecute(sInput, jtsIP, Integer.parseInt(jtsPort), 1);
		DAOCourierFileLog.mLogger.info("Outut XML to get final limit is  "+sInput);
	...
	{
		DAOCourierFileLog.mLogger.info("ProcessWI function for Falcon Document Utility started");
		...
		try
		{
			...
			//DAOCourierFileLog.mLogger.debug("QueueID: " + queueID);
		...
		if (wiList.size()>0)
		{
		    ...
		    DAOCourierFileLog.mLogger.debug("after printwriter:");
		    DAOCourierFileLog.mLogger.debug("File Path Auto:"+sFilePath);
			 writer.println("H|" + sDate + "|FALCON_CDP_DAILY_INC|" + outputFileName);
			 DAOCourierFileLog.mLogger.debug("after writer.println::");
			for (String wi : wiList)
			{
				...
				DAOCourierFileLog.mLogger.debug("APSelect Inputxml to fetch WI data is: "+sInputXML);

				String sOutputXML=WFNGExecute(sInputXML,jtsIP,Integer.parseInt(jtsPort),1);
				DAOCourierFileLog.mLogger.debug("APSelect OutputXML to fetch WI data is : "+sOutputXML);
			    ...
			    if("0".equals(sMainCode)){
			    	...
			    	DAOCourierFileLog.mLogger.info("B|" + strWIName + "|" + strPortalRefNo+ "|" + strAWBNo + "|" + strEmirateId + "|" + strCustomerName+"|"+strMobileNo);
	...
	{
		DAOCourierFileLog.mLogger.info("Starting loadWorkitem function");
		...
		DAOCourierFileLog.mLogger.debug("APSelect Inputxml: "+sInputXML);

		String sOutputXML=WFNGExecute(sInputXML,jtsIP,Integer.parseInt(jtsPort),1);
		DAOCourierFileLog.mLogger.debug("APSelect OutputXML: "+sOutputXML);
		XMLParser sXMLParser= new XMLParser(sOutputXML);
	    String sMainCode = sXMLParser.getValueOf("MainCode");
		List<String> wList=new ArrayList<String>();
		int sTotalRecords = Integer.parseInt(sXMLParser.getValueOf("TotalRetrieved"));
		DAOCourierFileLog.mLogger.debug("STotalRecords: "+sTotalRecords);
		String strWINAME="";
	...
	private void updateFlagInExttable(String WorkitemName)throws IOException,Exception{
		...
		DAOCourierFileLog.mLogger.info("Input XML to Upadte flag in table is : Generate File "+strInputXml);
	...
	private void updateExternalTable(String WorkitemNumber,String strAWB) throws IOException,Exception{
		...
		DAOCourierFileLog.mLogger.info("Input XML to update Flag in external table is "+sInput);
		String sOutput=WFNGExecute(sInput, jtsIP, Integer.parseInt(jtsPort), 1);
		DAOCourierFileLog.mLogger.info("outpput XML for external table update is "+sOutput);
	}
	private String getWorkitemNumberFromECRN(String ECRN) throws IOException,Exception{
		...
		String strOutput=WFNGExecute(sInput, jtsIP, Integer.parseInt(jtsPort), 1);
		DAOCourierFileLog.mLogger.info("Outut XML to get WI from ECRN is "+sInput);
	...
	private void insertRowInTable() throws IOException,Exception{
		...
		DAOCourierFileLog.mLogger.info("Input XML for insert courie details is "+strInput);
		String strOutputXML=WFNGExecute(strInput, jtsIP, Integer.parseInt(jtsPort), 1);
		DAOCourierFileLog.mLogger.info("Output XML to insert data in table");
	}
	...
	private void doneWorkItem(String wi_name,String values,Boolean... compeletFlag)
	{
		...
		try
		{
			...
			while(sessionCheckInt<loopCount)
			{
				...
				{
					DAOCourierFileLog.mLogger.error("Exception in Execute : " + e);
					sessionCheckInt++;
			...
			if (CommonMethods.getTagValues((String)executeXMLMapMethod.get("getWorkItemOutputXML"),"MainCode").equalsIgnoreCase("0")){
				...
				while(sessionCheckInt<loopCount)
				{
					...
					DAOCourierFileLog.mLogger.info("inputXml1 ---: "+executeXMLMapMethod.get("inputXml1"));
					DAOCourierFileLog.mLogger.debug("Output XML APCOMPLETE "+executeXMLMapMethod.get("inputXml1"));
					try
					{
					...
					DAOCourierFileLog.mLogger.info("outXml1 "+executeXMLMapMethod.get("outXml1"));
					sessionCheckInt++;
			...
			{
				DAOCourierFileLog.mLogger.info("Completed "+wi_name);
			}
			else
			{
				DAOCourierFileLog.mLogger.info("Problem in completion of "+wi_name+" ,Maincode :"+CommonMethods.getTagValues((String)executeXMLMapMethod.get("outXml1"),"MainCode"));
			}
		}
		catch(Exception e)
		{
			DAOCourierFileLog.mLogger.error("Exception in workitem done = " +e);

			final Writer result = new StringWriter();
			final PrintWriter printWriter = new PrintWriter(result);
			e.printStackTrace(printWriter);
			DAOCourierFileLog.mLogger.error("Exception Occurred in done wi : "+result);
		}
	}
	...
	private void updateExternalTable(String tablename, String columnname,String sMessage, String sWhere)
	{
		...
		while(sessionCheckInt<loopCount)
		{
			try
			{
				...
				DAOCourierFileLog.mLogger.debug("inputXmlcheckAPUpdate : " + inputXmlcheckAPUpdate);
				String outXmlCheckAPUpdate=null;
				outXmlCheckAPUpdate=WFNGExecute(inputXmlcheckAPUpdate,jtsIP,Integer.parseInt(jtsPort),1);
				DAOCourierFileLog.mLogger.info("outXmlCheckAPUpdate : " + outXmlCheckAPUpdate);
				...
				{
					DAOCourierFileLog.mLogger.error("Exception in ExecuteQuery_APUpdate updating "+tablename+" table");
					System.out.println("Exception in ExecuteQuery_APUpdate updating "+tablename+" table");
				}
				else
				{
					DAOCourierFileLog.mLogger.error("Succesfully updated "+tablename+" table");
			...
			{
				DAOCourierFileLog.mLogger.error("Inside create validateSessionID exception"+e);
			}
		}
	}
	...
	{
		DAOCourierFileLog.mLogger.debug("In History Caller method");
		...
		while(sessionCheckInt<loopCount)
		{
			try
			{
				if(workItemName!=null)
				{
					...
					DAOCourierFileLog.mLogger.debug("FormattedActionDateTime: "+formattedActionDateTime);
			...
			{
				e.printStackTrace();
				DAOCourierFileLog.mLogger.error("Exception in historyCaller of UpdateExpiryDate", e);
				sessionCheckInt++;
				waiteloopExecute(waitLoop);
				sessionId  = CommonConnection.getSessionID(DAOCourierFileLog.mLogger, false);
				continue;
		...
		{
			DAOCourierFileLog.mLogger.info("Insert Successful");
		}
		else
		{
			DAOCourierFileLog.mLogger.info("Insert Unsuccessful");
		}
		DAOCourierFileLog.mLogger.debug("Out History Caller method");
	}
	...
	{
		DAOCourierFileLog.mLogger.info("Start of function getEntryDatetimefromDB ");
		...
		while(sessionCheckInt<loopCount)
		{
			try 
			{
				...
				DAOCourierFileLog.mLogger.info("Getting getIntegrationErrorDescription from exttable table "+InputXMLEntryDate);
				outputXMLEntryDate = WFNGExecute(InputXMLEntryDate, jtsIP, Integer.parseInt(jtsPort), 1);
				DAOCourierFileLog.mLogger.info("OutputXML for getting getIntegrationErrorDescription from external table "+outputXMLEntryDate);
		...
		if (mainCodeEntryDate.equalsIgnoreCase("0")) 
		{
			try 
			{
				...
				DAOCourierFileLog.mLogger.debug("FormattedEntryDatetime: "+formattedEntryDatetime);

				DAOCourierFileLog.mLogger.info("newentrydatetime "+ formattedEntryDatetime);
			}
			catch (Exception e) 
			{
				e.printStackTrace();
	...
	{
		DAOCourierFileLog.mLogger.info("Starting loadWorkitem function for queueID -->"+queueID);
		List workItemList = null;
		String workItemListInputXML="";
		sessionCheckInt=0;
		String workItemListOutputXML="";
		DAOCourierFileLog.mLogger.info("loopCount aa:" + loopCount);
		DAOCourierFileLog.mLogger.info("lastWorkItemId aa:" + lastWorkItemId);
		DAOCourierFileLog.mLogger.info("lastProcessInstanceId aa:" + lastProcessInstanceId);
		while(sessionCheckInt<loopCount)
		{
			DAOCourierFileLog.mLogger.info("123 cabinet name..."+cabinetName);
			DAOCourierFileLog.mLogger.info("123 session id is..."+sessionId);
			...
			DAOCourierFileLog.mLogger.info("workItemListInputXML aa:" + workItemListInputXML);
			try
			{
			...
			DAOCourierFileLog.mLogger.info("workItemListOutputXML : " + workItemListOutputXML);
		...
		DAOCourierFileLog.mLogger.info("Exiting loadWorkitem function for queueID -->"+queueID);
		return workItemList;
	}
	...
	private List getWorkItems(String sessionId, String workItemListOutputXML, String[] last) throws NumberFormatException, Exception
	{
		...
		DAOCourierFileLog.mLogger.info("Starting getWorkitems function ");
		...
		if (size > 0)
		{
			...
			DAOCourierFileLog.mLogger.info("last[0] : "+last[0]);
		}
		DAOCourierFileLog.mLogger.info("Exiting getWorkitems function");
		return workItems;
	}
	...
	{
		DAOCourierFileLog.mLogger.info("Starting getWI function");
		...
		while(sessionCheckInt<loopCount)
		{
			...
			DAOCourierFileLog.mLogger.info("FetchAttributeInputXMl "+fetchAttributeInputXML);
			...
			DAOCourierFileLog.mLogger.info("fetchAttributeOutputXML "+fetchAttributeOutputXML);
			...
			{
				DAOCourierFileLog.mLogger.debug(" MapXML.getMainCode(fetchAttributeOutputXML) != 0 ");
			}
		}
		...
		{
			e.printStackTrace();
			DAOCourierFileLog.mLogger.debug("Inside catch of get wi function with exception.."+e);
		}
		DAOCourierFileLog.mLogger.info("Exiting getWI function");
		return wi;
	}
