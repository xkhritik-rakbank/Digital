...
public class DCCCourierFile implements Runnable{	
	...
	public void run()
	{
		...
		try
		{
			DCCCourierFileLog.setLogger();
			ngEjbClientFalconDocument = NGEjbClient.getSharedInstance();

			DCCCourierFileLog.mLogger.debug("Connecting to Cabinet.");

			int configReadStatus = readConfig();

			DCCCourierFileLog.mLogger.debug("configReadStatus "+configReadStatus);
			if(configReadStatus !=0)
			{
				DCCCourierFileLog.mLogger.error("Could not Read Config Properties [CMPDocument]");
				return;
			}

			cabinetName = CommonConnection.getCabinetName();
			DCCCourierFileLog.mLogger.debug("Cabinet Name: " + cabinetName);

			jtsIP = CommonConnection.getJTSIP();
			DCCCourierFileLog.mLogger.debug("JTSIP: " + jtsIP);

			jtsPort = CommonConnection.getJTSPort();
			DCCCourierFileLog.mLogger.debug("JTSPORT: " + jtsPort);

			smsPort = CommonConnection.getsSMSPort();
			DCCCourierFileLog.mLogger.debug("SMSPort: " + smsPort);			

			/*sleepIntervalInMin=Integer.parseInt(falconDocumentCofigParamMap.get("SleepIntervalInMin"));
			DCCCourierFileLog.mLogger.debug("SleepIntervalInMin: "+sleepIntervalInMin);

			attributeNames=falconDocumentCofigParamMap.get("AttributeNames").split(",");
			DCCCourierFileLog.mLogger.debug("AttributeNames: " + attributeNames);

			ExternalTable=falconDocumentCofigParamMap.get("ExtTableName");
			DCCCourierFileLog.mLogger.debug("ExternalTable: " + ExternalTable);

			destFilePath=falconDocumentCofigParamMap.get("destFilePath");
			DCCCourierFileLog.mLogger.debug("destFilePath: " + destFilePath);

			ErrorFolder=falconDocumentCofigParamMap.get("failDestFilePath");
			DCCCourierFileLog.mLogger.debug("ErrorFolder: " + ErrorFolder);

			volumeID=falconDocumentCofigParamMap.get("VolumeID");
			DCCCourierFileLog.mLogger.debug("VolumeID: " + volumeID);

			MaxNoOfTries=falconDocumentCofigParamMap.get("MaxNoOfTries");   //Not getting used anywhere Sajan
			DCCCourierFileLog.mLogger.debug("MaxNoOfTries: " + MaxNoOfTries);

			TimeIntervalBetweenTrialsInMin=Integer.parseInt(falconDocumentCofigParamMap.get("TimeIntervalBetweenTrialsInMin"));
			DCCCourierFileLog.mLogger.debug("TimeIntervalBetweenTrialsInMin: " + TimeIntervalBetweenTrialsInMin);*/
			
			sessionId = CommonConnection.getSessionID(DCCCourierFileLog.mLogger, false);
			if(sessionId.trim().equalsIgnoreCase(""))
			{
				DCCCourierFileLog.mLogger.debug("Could Not Connect to Server!");
			}
			else
			{
				DCCCourierFileLog.mLogger.debug("Session ID found: " + sessionId);
				while(true)
				{
		...
		catch (Exception e)
		{
			//e.printStackTrace();
			try{
				DCCCourierFileLog.mLogger.error("Exception Occurred in FALCON Document Thread: "+e+" Now runnning the utility again");
				Thread.sleep(1000);
				startFalconCourierUtility();
			}
			catch(Exception ex){
	...
	private int readConfig()
	{
		...
		catch (Exception e)
		{
			...
			DCCCourierFileLog.mLogger.error("Exception has occured while loading properties file "+e.getMessage());
			return -1 ;			
		}
		return 0;
	}
	...
	{
		DCCCourierFileLog.mLogger.info("ProcessWI function for Falcon Courier Utility started");
		...
		{
			DCCCourierFileLog.mLogger.error("Could Not Get Session ID "+sessionId);
			return;
		}
		...
		for(File file:listFiles){
			...
			for(int k=1;k<100;k++)
			{
				try
				{
					...
					DCCCourierFileLog.mLogger.info("Exception in reading xls "+e.getMessage());
					break;
				}
			}
			fis.close();
		...
		DCCCourierFileLog.mLogger.info("exiting ProcessWI function FALCON Document Utility");
	}
	...
	private String getFinalLimit(String workItemNumber) throws IOException,Exception{
		...
		DCCCourierFileLog.mLogger.info("Input XML to get Final limit is "+sInput);
		
		String strOutput=WFNGExecute(sInput, jtsIP, Integer.parseInt(jtsPort), 1);
		DCCCourierFileLog.mLogger.info("Outut XML to get final limit is  "+sInput);
	...
	{
		DCCCourierFileLog.mLogger.info("ProcessWI function for Falcon Document Utility started");
		...
		try
		{
			...
			//DCCCourierFileLog.mLogger.debug("QueueID: " + queueID);
		...
		if (wiList.size()>0)
		{
		    ...
		    DCCCourierFileLog.mLogger.debug("after printwriter:");
		    DCCCourierFileLog.mLogger.debug("File Path Auto:"+sFilePath);
			 writer.println("H|" + sDate + "|FALCON_CDP_DAILY_INC|" + outputFileName);
			 DCCCourierFileLog.mLogger.debug("after writer.println::");
			for (String wi : wiList)
			{
				...
				DCCCourierFileLog.mLogger.debug("APSelect Inputxml to fetch WI data is: "+sInputXML);

				String sOutputXML=WFNGExecute(sInputXML,jtsIP,Integer.parseInt(jtsPort),1);
				DCCCourierFileLog.mLogger.debug("APSelect OutputXML to fetch WI data is : "+sOutputXML);
			    ...
			    if("0".equals(sMainCode)){
			    	...
			    	DCCCourierFileLog.mLogger.info("B|" + strWIName + "|" + strPortalRefNo+ "|" + strAWBNo + "|" + strEmirateId + "|" + strCustomerName+"|"+strMobileNo);
	...
	{
		DCCCourierFileLog.mLogger.info("Starting loadWorkitem function");
		...
		DCCCourierFileLog.mLogger.debug("APSelect Inputxml: "+sInputXML);

		String sOutputXML=WFNGExecute(sInputXML,jtsIP,Integer.parseInt(jtsPort),1);
		DCCCourierFileLog.mLogger.debug("APSelect OutputXML: "+sOutputXML);
		XMLParser sXMLParser= new XMLParser(sOutputXML);
	    String sMainCode = sXMLParser.getValueOf("MainCode");
		List<String> wList=new ArrayList<String>();
		int sTotalRecords = Integer.parseInt(sXMLParser.getValueOf("TotalRetrieved"));
		DCCCourierFileLog.mLogger.debug("STotalRecords: "+sTotalRecords);
		String strWINAME="";
	...
	private void updateFlagInExttable(String WorkitemName)throws IOException,Exception{
		...
		DCCCourierFileLog.mLogger.info("Input XML to Upadte flag in table is : Generate File "+strInputXml);
	...
	private void updateExternalTable(String WorkitemNumber,String strAWB) throws IOException,Exception{
		...
		DCCCourierFileLog.mLogger.info("Input XML to update Flag in external table is "+sInput);
		String sOutput=WFNGExecute(sInput, jtsIP, Integer.parseInt(jtsPort), 1);
		DCCCourierFileLog.mLogger.info("outpput XML for external table update is "+sOutput);
	}
	private String getWorkitemNumberFromECRN(String ECRN) throws IOException,Exception{
		...
		String strOutput=WFNGExecute(sInput, jtsIP, Integer.parseInt(jtsPort), 1);
		DCCCourierFileLog.mLogger.info("Outut XML to get WI from ECRN is "+sInput);
	...
	private void insertRowInTable() throws IOException,Exception{
		...
		DCCCourierFileLog.mLogger.info("Input XML for insert courie details is "+strInput);
		String strOutputXML=WFNGExecute(strInput, jtsIP, Integer.parseInt(jtsPort), 1);
		DCCCourierFileLog.mLogger.info("Output XML to insert data in table");
	}
	...
	private void doneWorkItem(String wi_name,String values,Boolean... compeletFlag)
	{
		...
		try
		{
			...
			while(sessionCheckInt<loopCount)
			{
				...
				{
					DCCCourierFileLog.mLogger.error("Exception in Execute : " + e);
					sessionCheckInt++;
			...
			if (CommonMethods.getTagValues((String)executeXMLMapMethod.get("getWorkItemOutputXML"),"MainCode").equalsIgnoreCase("0")){
				...
				while(sessionCheckInt<loopCount)
				{
					...
					DCCCourierFileLog.mLogger.info("inputXml1 ---: "+executeXMLMapMethod.get("inputXml1"));
					DCCCourierFileLog.mLogger.debug("Output XML APCOMPLETE "+executeXMLMapMethod.get("inputXml1"));
					try
					{
					...
					DCCCourierFileLog.mLogger.info("outXml1 "+executeXMLMapMethod.get("outXml1"));
					sessionCheckInt++;
			...
			{
				DCCCourierFileLog.mLogger.info("Completed "+wi_name);
			}
			else
			{
				DCCCourierFileLog.mLogger.info("Problem in completion of "+wi_name+" ,Maincode :"+CommonMethods.getTagValues((String)executeXMLMapMethod.get("outXml1"),"MainCode"));
			}
		}
		catch(Exception e)
		{
			DCCCourierFileLog.mLogger.error("Exception in workitem done = " +e);

			final Writer result = new StringWriter();
			final PrintWriter printWriter = new PrintWriter(result);
			e.printStackTrace(printWriter);
			DCCCourierFileLog.mLogger.error("Exception Occurred in done wi : "+result);
		}
	}
	...
	private void updateExternalTable(String tablename, String columnname,String sMessage, String sWhere)
	{
		...
		while(sessionCheckInt<loopCount)
		{
			try
			{
				...
				DCCCourierFileLog.mLogger.debug("inputXmlcheckAPUpdate : " + inputXmlcheckAPUpdate);
				String outXmlCheckAPUpdate=null;
				outXmlCheckAPUpdate=WFNGExecute(inputXmlcheckAPUpdate,jtsIP,Integer.parseInt(jtsPort),1);
				DCCCourierFileLog.mLogger.info("outXmlCheckAPUpdate : " + outXmlCheckAPUpdate);
				...
				{
					DCCCourierFileLog.mLogger.error("Exception in ExecuteQuery_APUpdate updating "+tablename+" table");
					System.out.println("Exception in ExecuteQuery_APUpdate updating "+tablename+" table");
				}
				else
				{
					DCCCourierFileLog.mLogger.error("Succesfully updated "+tablename+" table");
			...
			{
				DCCCourierFileLog.mLogger.error("Inside create validateSessionID exception"+e);
			}
		}
	}
	...
	{
		DCCCourierFileLog.mLogger.debug("In History Caller method");
		...
		while(sessionCheckInt<loopCount)
		{
			try
			{
				if(workItemName!=null)
				{
					...
					DCCCourierFileLog.mLogger.debug("FormattedActionDateTime: "+formattedActionDateTime);
			...
			{
				e.printStackTrace();
				DCCCourierFileLog.mLogger.error("Exception in historyCaller of UpdateExpiryDate", e);
				sessionCheckInt++;
				waiteloopExecute(waitLoop);
				sessionId  = CommonConnection.getSessionID(DCCCourierFileLog.mLogger, false);
				continue;
		...
		{
			DCCCourierFileLog.mLogger.info("Insert Successful");
		}
		else
		{
			DCCCourierFileLog.mLogger.info("Insert Unsuccessful");
		}
		DCCCourierFileLog.mLogger.debug("Out History Caller method");
	}
	...
	{
		DCCCourierFileLog.mLogger.info("Start of function getEntryDatetimefromDB ");
		...
		while(sessionCheckInt<loopCount)
		{
			try 
			{
				...
				DCCCourierFileLog.mLogger.info("Getting getIntegrationErrorDescription from exttable table "+InputXMLEntryDate);
				outputXMLEntryDate = WFNGExecute(InputXMLEntryDate, jtsIP, Integer.parseInt(jtsPort), 1);
				DCCCourierFileLog.mLogger.info("OutputXML for getting getIntegrationErrorDescription from external table "+outputXMLEntryDate);
		...
		if (mainCodeEntryDate.equalsIgnoreCase("0")) 
		{
			try 
			{
				...
				DCCCourierFileLog.mLogger.debug("FormattedEntryDatetime: "+formattedEntryDatetime);

				DCCCourierFileLog.mLogger.info("newentrydatetime "+ formattedEntryDatetime);
			}
			catch (Exception e) 
			{
				e.printStackTrace();
	...
	{
		DCCCourierFileLog.mLogger.info("Starting loadWorkitem function for queueID -->"+queueID);
		List workItemList = null;
		String workItemListInputXML="";
		sessionCheckInt=0;
		String workItemListOutputXML="";
		DCCCourierFileLog.mLogger.info("loopCount aa:" + loopCount);
		DCCCourierFileLog.mLogger.info("lastWorkItemId aa:" + lastWorkItemId);
		DCCCourierFileLog.mLogger.info("lastProcessInstanceId aa:" + lastProcessInstanceId);
		while(sessionCheckInt<loopCount)
		{
			DCCCourierFileLog.mLogger.info("123 cabinet name..."+cabinetName);
			DCCCourierFileLog.mLogger.info("123 session id is..."+sessionId);
			...
			DCCCourierFileLog.mLogger.info("workItemListInputXML aa:" + workItemListInputXML);
			try
			{
			...
			DCCCourierFileLog.mLogger.info("workItemListOutputXML : " + workItemListOutputXML);
		...
		DCCCourierFileLog.mLogger.info("Exiting loadWorkitem function for queueID -->"+queueID);
		return workItemList;
	}
	...
	private List getWorkItems(String sessionId, String workItemListOutputXML, String[] last) throws NumberFormatException, Exception
	{
		...
		DCCCourierFileLog.mLogger.info("Starting getWorkitems function ");
		...
		if (size > 0)
		{
			...
			DCCCourierFileLog.mLogger.info("last[0] : "+last[0]);
		}
		DCCCourierFileLog.mLogger.info("Exiting getWorkitems function");
		return workItems;
	}
	...
	{
		DCCCourierFileLog.mLogger.info("Starting getWI function");
		...
		while(sessionCheckInt<loopCount)
		{
			...
			DCCCourierFileLog.mLogger.info("FetchAttributeInputXMl "+fetchAttributeInputXML);
			...
			DCCCourierFileLog.mLogger.info("fetchAttributeOutputXML "+fetchAttributeOutputXML);
			...
			{
				DCCCourierFileLog.mLogger.debug(" MapXML.getMainCode(fetchAttributeOutputXML) != 0 ");
			}
		}
		...
		{
			e.printStackTrace();
			DCCCourierFileLog.mLogger.debug("Inside catch of get wi function with exception.."+e);
		}
		DCCCourierFileLog.mLogger.info("Exiting getWI function");
		return wi;
	}
