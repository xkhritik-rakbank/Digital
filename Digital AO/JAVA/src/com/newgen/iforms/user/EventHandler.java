...
public class EventHandler extends DigitalAO_Common  implements IFormServerEventHandler
{	
	...
	{	
		//CSR_OCC.setLogger();
		DigitalAO.mLogger.info("Inside executeServerEvent() ak 101 ---control: " + control + "\nevent: " + event + "\nStringData: "
				+ Stringdata);
		...
		{
			return new DigitalAO_FormLoad().formLoadEvent(iformObj,control,event,Stringdata);
		}
		else if("introducedone".equalsIgnoreCase(event))
		{
			return new DigitalAO_IntroDone().onIntroduceDone(iformObj,control,event,Stringdata);
		}
		else if("Change".equalsIgnoreCase(event))
		{
			return new DigitalAO_Change().changeEvent(iformObj,control,event,Stringdata);
		}
